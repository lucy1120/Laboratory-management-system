	...
	<script type="text/javascript">
		...
		function checkDel(uid){
  			if(window.confirm("是否删除该用户？")){
  				window.location.href="SadminManagerServlet?act=deleteUser&uid="+uid;
  			}
  		}
	</script>
</head>
<body>
	...
	<form action="SadminManagerServlet?act=selectUser&&del=del" method="post">
	...
	<table>
		...
		<tr onmousemove="changeColor(this)" onmouseout="changeColor1(this)">
			...
			<td>${n.saclassroom}</td>
			<td>
				<a href="javaScript:checkDel('${n.said}')" style="text-decoration:none;color:#2996e3;">删除</a>
			</td>
		</tr>
		</c:forEach>
		<tr>
		<c:if test="${pageCur != null}">
			<td colspan="10" align="right">
				...
				第${pageCur}页&nbsp;&nbsp;
				<c:url var="url_pre" value="SadminManagerServlet?aact=${aact }&&uname1=${uname1}&&umajor=${umajor}&&act=selectUser&&del=del">
					<c:param name="pageCur" value="${pageCur - 1 }"/>
				</c:url>
				<c:url var="url_next" value="SadminManagerServlet?aact=${aact }&&uname1=${uname1}&&umajor=${umajor}&&act=selectUser&&del=del">
					<c:param name="pageCur" value="${pageCur + 1 }"/>
				</c:url>
				<!-- 第一页没有上一页 -->
