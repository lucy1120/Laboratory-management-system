...
@WebServlet("/SadminManagerServlet")
public class SadminManagerServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("select".equals(act)){
			rds = request.getRequestDispatcher("Admin/SadminSelect.jsp");
			...
			if("condiction".equals(aact)){
				...
				list = abiz.conditionSelectUsers(uname, umajor);
			...
			//删除
			if("select".equals(aact) || "condiction".equals(aact) && "del".equals(del)){
				rds = request.getRequestDispatcher("Admin/SadminDelete.jsp");
			}
			rds.forward(request, response);
			...
			if(abiz.deleteUser(uid)){
				out.println("<script type=\"text/javascript\">\r\n"
						...
						+ "        }else{\r\n"
						+ "            window.location.href = 'SadminManagerServlet?act=delete';\r\n"
						+ "        }\r\n"
						+ "    }\r\n"
	...
	//		request.setAttribute("userList", list);
			...
			try {
				...
				out.println("<script type=\"text/javascript\">\r\n"
						...
						+ "            setTimeout(this.jump,1000);\r\n"
						+ "        }else{\r\n"
						+ "            window.location.href = 'SadminManagerServlet?act=add';\r\n"
						+ "        }\r\n"
						+ "    }\r\n"
						+ "    setTimeout(jump,1000);\r\n"
						+ "</script>");
			...
			if(abiz.IsUser(uid).size() == 0 && !"".equals(uname) && !"".equals(upwd) && !"".equals(usex)
				...
				if(abiz.addUser(user)) {
					out.println("<script type=\"text/javascript\">\r\n"
							...
							+ "            setTimeout(this.jump,1000);\r\n"
							+ "        }else{\r\n"
							+ "            window.location.href = 'SadminManagerServlet?act=select';\r\n"
							+ "        }\r\n"
							+ "    }\r\n"
							+ "    setTimeout(jump,1000);\r\n"
							+ "</script>");
...
//				response.setHeader("refresh", "20; url=UserManagerServlet?act=add");
			...
			list = abiz1.selectComputerroomExceptOnduty();
			request.setAttribute("computerroomList", list);
			rds = request.getRequestDispatcher("Admin/SadminAdd.jsp");
			rds.forward(request, response);
		}else if("modify".equals(act)){
			rds = request.getRequestDispatcher("Admin/SadminModify.jsp");
			rds.forward(request, response);
		...
		//				response.setHeader("refresh", "3; url=UserManagerServlet?act=select");
						...
						out.println("<script type=\"text/javascript\">\r\n"
								...
								+ "            setTimeout(this.jump,1000);\r\n"
								+ "        }else{\r\n"
								+ "            window.location.href = 'SadminManagerServlet?act=modify';\r\n"
								+ "        }\r\n"
								+ "    }\r\n"
								+ "    setTimeout(jump,1000);\r\n"
								+ "</script>");
